                ...
                if (result.status === 'Error') {
                    ...
                    oTable = $('#tbl-prejob_list').DataTable({
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row, meta) {
                                //    return '<a href="http://192.168.1.247/intranet/report/cn-rpt/Pages/RPT_CN/RPT_CN_REINVOICE?cn_pre_job_jobno=' + val['cn_pre_job_jobno'] + '&salefile_number=' + val['salefile_number'] + '&gbarcode=' + val['saletra_item_barcode'] + '&salefile_invcode=' + val['salefile_invcode'] + '" target="_blank"><b>' + val['salefile_number'] + '<b></a>'
                                return data
                            }
                        }, //1
                        {
            ...
            $('#btn-save_form').on("click", function (e) {
                ...
                $('#frm_data').parsley().on('form:submit', function () {
                    ...
                    let add_data = {
                        ...
                        cn_pre_job_item_barcode: $('#frm_data').find('#item_list').val(),
                        cn_pre_job_item_name: $('#frm_data').find('#item_list').attr("data-name"),
                        cn_pre_job_item_spcode: $('#frm_data').find('#item_list').attr("data-spcode"),
                    ...
                    fetch(url_cn_branch_pre_job_create, {
                        ...
                        if (data.status === 'Error') {
                            ...
                            toastr.success('Save Successfully!', async function () {
                                ...
                                $('#frm_data input, #frm_data textarea:not(#job_status , #created_by)').val('')
