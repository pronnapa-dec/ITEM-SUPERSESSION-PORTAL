...
// P SOMO
let customer_profile_detail186_get = connect_url + '/api/CustomerProfile_Detail186_Get';
...
$.init = function () {
    ...
    setTimeout(function () {
        ...
        $('#emmas_eprovinc, #emmas_eamphur, #emmas_etumbol, #customer_eprovinc2, #customer_eamphur2, #customer_etumbol2').select2({
        ...
        $('#customer_delivery_route, #customer_delivery_subroute, #province, #amphur, #district').select2({
            placeholder: 'Choose one',
            width: '493px',
            searchInputPlaceholder: 'Search',
    ...
    $(".province").on('change', function () {
        if ($(this).attr('id') == 'emmas_eprovinc') {
            $("#emmas_eamphur option").remove();
            $("#emmas_eamphur").append('<option value="">Please select..</option>').attr("value", ''); //kung edit 2021-12-28
            $.Amphur_Get($(this).val(), "emmas_eamphur");
            $("#emmas_etumbol").val('').trigger('change');;
            $('#emmas_ezip').val('');

        } else if ($(this).attr('id') == 'customer_eprovinc2') {
            $("#customer_eamphur2 option").remove();
            $("#customer_eamphur2").append('<option value="">Please select..</option>').attr("value", ''); //kung edit 2021-12-28
            $.Amphur_Get($(this).val(), "customer_eamphur2");
            $("#customer_etumbol2").val('').trigger('change');;
            $('#customer_ezip2').val('');
    ...
    $(".amphur").on('change', function () {
        ...
        } else if ($(this).attr('id') == 'customer_eamphur2') {

            $.District_Get($(this).val(), 'customer_etumbol2');
            $('#customer_ezip2').val($('#customer_eamphur2 option:selected').attr('data_zip'))

        } else if ($(this).attr('id') == 'amphur') {

            $.District_Get($(this).val(), 'district');
            $('#cus_postcode').val($('#amphur option:selected').attr('data_zip'))
    ...
    $.ajax({
        ...
        success: function (respond) {
            console.log(respond)
            $('#customer_delivery_prioritytype')
                .append($("<option value=''>Please select..</option>")
                    .attr("value", respond.lov_id)
                    .text(item.lov1));
        }
    });
};
    ...
    fetch(customer_detail + '?code=' + citem['code']).then(function (response) {
        ...
        if (result.length > 0) {
            ...
            $.each(result.data, function (key, val) {
                ...
                $('#customerinfo').find('#salegrch_grouprank').val(val['grouprank']).prop('disabled', true);/*ใช้อันไหน??*/
                ...
                //start kung edit วิธีการ Get data ที่อยู่มาโชว์ (2022/01/14)
                ...
                $("#customerinfo").find("#emmas_eprovinc option:contains(" + val['eprovinc'] + ")").attr('selected', true).trigger('change');
                $("#customerinfo").find("#customer_eprovinc2 option:contains(" + val['customer_eprovinc2'] + ")").attr('selected', true).trigger('change');

                setTimeout(function () {
                    $("#customerinfo").find("#emmas_eamphur option:contains(" + val['eamphur'] + ")").attr('selected', true).trigger('change');
                    $("#customerinfo").find("#customer_eamphur2 option:contains(" + val['customer_eamphur2'] + ")").attr('selected', true).trigger('change');
                }, 300)
                setTimeout(function () {
                    $("#customerinfo").find("#emmas_etumbol option:contains(" + val['etumbol'] + ")").attr('selected', true).trigger('change');
                    $("#customerinfo").find("#customer_etumbol2 option:contains(" + val['customer_etumbol2'] + ")").attr('selected', true).trigger('change');
                }, 600)

                $('#customerinfo').find('#emmas_ezip').val(val['ezip']);
                $('#customerinfo').find('#customer_ezip2').val(val['customer_ezip2']);
                //end kung edit วิธีการ Get data ที่อยู่มาโชว์

                //#Tab2 - ข้อมูลด้านการขาย

                $('#saleinfo').find('#samas_salecode').val(val['salecode']);
                $('#saleinfo').find('#samas_salename').val(val['salename']);
                $('#saleinfo').find('#emmas_egdis').val(val['egdis']);
                $('#saleinfo').find('#samas_storemanager').val(val['StoreManager']);
                $('#saleinfo').find('#samas_salechannel').val(val['salechannel']);
                $('#saleinfo').find('#salegrch_branch').val(val['Branch']);
                $('#saleinfo').find('#samas_salegroup').val(val['salegroup']);
                $('#saleinfo').find('#salegrch_area').val(val['Area']);
                $('#saleinfo').find('#salegrch_salearea').val(val['SaleArea']);
                $('#saleinfo').find('#samas_salemanager').val(val['SaleManager']);
                $('#saleinfo').find('#samas_salesupervisor').val(val['SaleSupervisor']);
                $('#saleinfo').find('#samas_saleexecutive').val(val['SaleRepresentative']);
                $('#saleinfo').find('#samas_salesupervisorsupport').val(val['SupportSupervisor']);
                $('#saleinfo').find('#samas_partspecialist').val(val['SaleSupport']);
                ...
                $('#accountinfo').find('#emmas_euserid').val(val['euserid']);
                $('#accountinfo').find('#emmas_evat').val(val['evat']);
                $('#accountinfo').find('#emmas_egroup').val(val['egroup']);
                $('#accountinfo').find('#emmas_ekcust').val(val['ekcust']);
                $('#accountinfo').find('#emmas_egroupkE').val(val['egroupkE']);
                $('#accountinfo').find('#emmas_egroupk').val(val['egroupk']);
                $('#accountinfo').find('#emmas_ekbill').val(val['ekbill']);
                $('#accountinfo').find('#emmas_ecrcut').val(val['ecrcut']);
                $('#accountinfo').find('#emmas_ekcheck').val(val['ekcheck']);
                $('#accountinfo').find('#emmas_ecredit').val(val['ecredit']);
                $('#accountinfo').find('#emmas_ecredit2').val(val['ecredit2']);
                $('#accountinfo').find('#emmas_eamount').val(val['eamount']);
                $('#accountinfo').find('#emmas_eamountvat').val(val['eamountvat']);

                $('#accountinfo').find('#emmas_taxid').val(val['taxid']);
                $('#accountinfo').find('#emmas_edue').val(val['edue']);
    ...
    $('#btn-item-create').off().on('click', function (e) {  //kung edit 2021-12-28
        ...
        if ($('#frm_customer').parsley().isValid()) {
            ...
            let add_data = {
                ...
                route: $('#customer_delivery_route').val(),
                subroute: $('#customer_delivery_subroute').val()
...
$.DetailsAddr = async function (citem, cusitem) {
    ...
    // kung เพิ่ม 3 field 2021/01/18
    $('#location_name').val(citem['location_name']);
    $('#customer_delivery_route').val(citem['route']).trigger('change');

    setTimeout(function () {
        $('#amphur').val(citem['eamphur_id']).trigger('change');
        $('#customer_delivery_subroute').val(citem['subroute']).trigger('change');

        setTimeout(function () {
            $('#district').val(citem['etumbol_id']).trigger('change');
        }, 100);
    }, 300); 
    //end kung เพิ่ม 3 field 2021/01/18

    $('#cus_postcode').val(citem['ezip']);
...
//kung edit ไม่ใช้ function นี้แล้ว (2022/01/14)
...
$.Province_Get = async function (province_name) {

    let province_get_api = new URL(province_get);

    //edit ไม่ใช่ code ส่วนนี้แล้ว (2022/01/14)
...
$.Edit_Trp = async function (citem, addrcitem, cusitem) {
    ...
    $('#btn-edit-trp').on('click', function (e) {
